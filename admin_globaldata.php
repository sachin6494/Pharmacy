...
$sql="INSERT INTO stock_np(drug_name,category,description,company,supplier,city,quantity,cost,status,date_supplied)
...
if(mysqli_query($conn, $sql)) {header("location:http://".$_SERVER['HTTP_HOST'].dirname($_SERVER['PHP_SELF'])."/stock_pharmacist.php");
}else{
...
<?php echo $message;
		...
		*/
		...
		$sql = "SELECT * FROM stock_np UNION ALL SELECT * FROM stock_ca UNION ALL SELECT * FROM stock_dr  order by stock_id";
